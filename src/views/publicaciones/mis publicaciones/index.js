  ...
  const getPublications = async () => {
    ...
    try {
      ...
      const response = await fetchRequest(
        `/publicaciones/obtener_mis_publicaciones/${usuario.idUsuario}?page=${page}&limit=${rowsPerPage}&showAll=1`,
        "POST",
        filtersAndParams
      );
      ...
      const dataFormated = rows.map((publicacion) => ({
        ...
        publishSwitch: publicacion.estadoPublicacion,
        date: dayjs(publicacion.fechaCreacion)
          .locale(es)
          .format("DD [de] MMMM [de] YYYY"),
        lastModification: dayjs(publicacion.ultimaActualizacion)
          .locale(es)
          .format("DD [de] MMMM [de] YYYY"),
        rating: publicacion.rating,
        comments: publicacion.totalComentarios,
        publicedBy: publicacion.nombreCreador,
        avatar: publicacion.imagenCreador,
